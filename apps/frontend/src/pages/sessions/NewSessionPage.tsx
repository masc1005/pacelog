...
export const NewSessionPage: React.FC = () => {
  ...
  useEffect(() => {
    if (stateKey) {
      ...
      if (stateKey === 'jiujitsu') setMetrics({ trainingType: 'technique', gi: true, roundsCount: 5, beltRank: 'white', beltDegree: 0 });
  ...
  const handleNext = () => {
    if (step === 1 && sportKey) {
      ...
      if (sportKey === 'jiujitsu' && !metrics.trainingType) setMetrics({ trainingType: 'technique', gi: true, roundsCount: 5, beltRank: 'white', beltDegree: 0 });
  ...
  return (
    <div className="flex flex-col gap-6 font-sans max-w-2xl mx-auto w-full">
      <div className="flex items-center justify-between border-b border-[#1F2937] pb-4">
        ...
        <Badge variant={step === 3 ? 'sage' : 'neutral'}>
          {step === 3 ? 'FINALIZAÇÃO' : 'RASCUNHO'}
        </Badge>
      </div>
            ...
            {sportKey === 'jiujitsu' && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div className="col-span-2 sm:col-span-3">
                  <Input
                    label="Foco Técnico / Posições (ex: Raspagem de gancho, Passagem emborcada)"
                    type="text"
